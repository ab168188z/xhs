...
;(function (){
	...
	var $__cookie = function(name, value, options) {
	...
	var userid = getQueryString().bfsemuserid; // 用户id
	var userdurl = '//api-ws.zeifeng.cn'; // 接口域名
	var wordid = getQueryString().wordId;
	wordid = (!wordid || wordid === '{keywordid}') ? '': wordid;
	var creativeid = getQueryString().creativeid
	var cyid = (!creativeid || creativeid === '{creativeid}') ? '': creativeid;
	var bbpid = +getQueryString().bbpid || 0;
	var k = 'v_'+ wordid +'_'+ cyid;
	var qhclickid = getQueryString().qhclickid;
	var bd_vid = getQueryString().bd_vid;
	var soft_id = location.href.match(/\/(\d*)\.html/) ? location.href.match(/\/(\d*)\.html/)[1] : 1111;
	function getCode() {
		var CODE = '';
		if(!wordid || !userid) {
			console.log('=======没有wordId或者bfsemuserid=======')
			return ''
		}
		var xhr = new XMLHttpRequest();
		xhr.open('POST', userdurl + '/getCode', false);
		xhr.onload = function () {
			var expireTime = new Date()
			expireTime.setTime(new Date(new Date().setHours(0, 0, 0, 0) + 24 * 60 * 60 * 1000 - 1).getTime())
			if (xhr.status === 200) {
				try {
					var responseData = JSON.parse(xhr.response);
					CODE = responseData.data;
				}catch (e){}
				$__cookie(k, CODE, { expires: expireTime, path: '/' })
			}
		};
		var formData = new FormData()
		formData.append('userid', userid)
		formData.append('wordid', wordid)
		formData.append('cyid', cyid)
		if(bd_vid){
			formData.append('clickid', bd_vid + '')
		}else if(qhclickid){
			formData.append('clickid', qhclickid + '')
		}else{
			formData.append('code', $__cookie(k) || '')
		}
		formData.append('bbpid', bbpid)
		formData.append('soft_id', soft_id)
		xhr.send(formData);
		return CODE
	}
	var CODE = getCode();
	function t() {
		...
		img.src = userdurl + '/thirdstore.gif?user_id='+ userid +'&soft_id='+ soft_id +'&wordid=' + wordid + '&cyid=' + cyid + '&code=' + CODE + '&clickid='+ clickid + '&is_btn='+ 1;
		img = null
	}
// 下载按钮
$(document).on('click', '.main-footdown-btn, .safe-btn-download', function () {
	 t();
	});
})();